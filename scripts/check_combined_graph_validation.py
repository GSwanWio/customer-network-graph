...
def main() -> None:
	...
	print("Combined graph validation passed.")
	print(f"Combined nodes: {len(combined_graph['combined_nodes'])}")
	print(f"Combined edges: {len(combined_graph['combined_edges'])}")
	print(f"Combined components: {len(combined_graph['combined_components'])}")

	component = combined_graph["combined_components"].iloc[0]
	print(f"Graph node count: {component['graph_node_count']}")
	print(f"Entity node count: {component['entity_node_count']}")
	print(f"EID node count: {component['eid_node_count']}")
	print(f"Counterparty account node count: {component['counterparty_account_node_count']}")
	print(f"Seed entity count: {component['seed_entity_count']}")
	print(f"Counterparty expansion seed entity count: {component['counterparty_expansion_seed_entity_count']}")
	print(f"Graph edge count: {component['graph_edge_count']}")
	print(f"Component profile: {component['component_profile']}")
